...
HttpResponse ProfileHandler::HandleCreateProfile(const HttpRequest & request, const RouteContext & context)
{
  try {
    ...
    json response = {
      ...
      {"created_at", std::chrono::duration_cast<std::chrono::seconds>(profile.CreatedAt.time_since_epoch()).count()},
      {"updated_at", std::chrono::duration_cast<std::chrono::seconds>(profile.UpdatedAt.time_since_epoch()).count()}
    };
